...
###chmod -R 777 /in/
###nohup sh doc.sh -p 10 -d /in/sra/ -g genome.fna -f genome.gff &
...
do
    if [ ! -f /in/stringtie_result/${ID}.table ] ; then 
        ...
        -o /in/stringtie_result/${ID}.gtf -p ${threads} -e -B &
        pids="$!"
        echo
        echo $(date '+%d %B %T') ...... Processing stringtie PID: $pids
        wait $pids
        awk '{print $6,$12}' /in/stringtie_result/t_data.ctab > /in/stringtie_result/${ID}.fpkm
        sed -i 's/t_name/aaa/g' /in/stringtie_result/${ID}.fpkm
        sort -k1 /in/stringtie_result/${ID}.fpkm > /in/merge_table/${ID}.sortfpkm
        sed -i 's/aaa/KEY/g' /in/merge_table/${ID}.sortfpkm
        sed -i "s/FPKM/${ID}/g" /in/merge_table/${ID}.sortfpkm
        sed -i 's/ /,/g' /in/merge_table/${ID}.sortfpkm
        rm -rf /in/stringtie_result/tmp* /in/stringtie_result/*ctab
    fi
done
...
/in/csvtk join -f 1 /in/merge_table/*sortfpkm -O --na 0 > /in/merge_table/fpkm.table
